...
public final class AutoBuyerCommand extends Command {
    private final AutoBuyerTerminalService terminalService;
    private final MessageService messages;

    public AutoBuyerCommand(AutoBuyerTerminalService terminalService, MessageService messages) {
        super("setautobuyer", messages.tr(MessageKey.AUTO_BUYER_SET_DESCRIPTION), "midas.command.autobuyer");
    ...
    public void prepareCommandTree(CommandTree tree) {
        tree.getRoot()
                .exec(context -> {
                    ...
                    terminalService.startSelection(context.getSender().asPlayer());
                    if (terminalService.isSelecting(context.getSender().asPlayer())) {
                        context.getSender().sendMessage(messages.tr(context.getSender().asPlayer(), MessageKey.AUTO_BUYER_SELECT_BIND));
                    } else {
